...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        #posactions = self.problem.actions(encode_state(self.fs,self.problem.state_map))
        for action in self.all_actions:
            anode = PgNode_a(action)
            #check if all prenodes are in s level
    ...
    def add_literal_level(self, level):
        ...
        #get previous level action nodes
        anodes = self.a_levels[level-1]
        for anode in anodes:
            for effect in anode.effnodes:
                slevelnode = PgNode_s(effect.symbol, effect.is_pos)
                slevelnode.parents.add(anode)
                anode.children.add(slevelnode)
                self.s_levels[level].add(slevelnode)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effectadd in node_a1.action.effect_add:
            if effectadd in node_a2.action.effect_rem:
                return True

        for effectadd in node_a2.action.effect_add:
            if effectadd in node_a1.action.effect_rem:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effectadd in node_a1.action.effect_add:
            if effectadd in node_a2.action.precond_neg:
                return True

        for effectadd in node_a2.action.effect_add:
            if effectadd in node_a1.action.precond_neg:
                return True

        for effectrem in node_a1.action.effect_rem:
            if effectrem in node_a2.action.precond_pos:
                return True

        for effectrem in node_a2.action.effect_rem:
            if effectrem in node_a1.action.precond_pos:
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #from forums check for parent not precondition
        for posprecond_nodeparent in node_a1.parents:
            for posprecond2 in node_a2.parents:
                if posprecond_nodeparent.is_mutex(posprecond2):
                    return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for actionnode in node_s1.parents:
            for actionnode2 in node_s2.parents:
                if not actionnode.is_mutex(actionnode2):
                    return False
        return True
